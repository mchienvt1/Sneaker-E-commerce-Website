...
export default function ManageOutStandingProduct() {
  const navigate = useNavigate();
  const [arrProduct, setArrProduct] = useState([]);

  const fetchAllProducts = async () => {
    const result = await axios.get(`http://localhost/BE/?c=outStanding&a=list&search`);
  ...
  const handleClickDelete = (product) => {
    ...
    axios
      .delete(`http://localhost/BE/?c=outStanding&a=delete&productId=${product.id}`)
      .then((result) => {
  ...
  return (
    <div className={style.container}>
      <Header route={'manageOutStandingProduct'}/>
